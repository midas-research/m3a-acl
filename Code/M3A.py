...
def createModelV(emd1, emd2, heads, dimFF, maxlen, maxSpeaker):
...
def createModelC(emd1, emd2, heads, dimFF, maxlen, maxSpeaker):
  embed_dim1 = emd1  # Embedding size for Text 
  embed_dim2 = emd2  # Embedding size for Audio
  num_heads = heads  # Number of attention heads
  ff_dim = dimFF  # Hidden layer size in feed forward network inside transformer
...
path_to_files = '' # insert path with last /
...
for i in range(3):
  ...
  mc = tf.keras.callbacks.ModelCheckpoint("Model.h5", monitor='val_loss', verbose=0, save_best_only=True)
  model = createModelV(768, 62, 3, 16, maxLen,maxSpeaker)
  model.compile(loss='mean_squared_error', optimizer=Adam())
  out = model.fit([X_text_Train,X_audio_Train,X_pos_Train,X_speak_Train], YTrain, batch_size=64, epochs=500, validation_data=([X_text_Test,X_audio_Test,X_pos_Test,X_speak_Test],YTest), verbose=0, callbacks=[mc])
  depen = {'MultiHeadSelfAttention': MultiHeadSelfAttention,'TransformerBlock': TransformerBlock} 
  model = load_model("Model.h5", custom_objects=depen)
